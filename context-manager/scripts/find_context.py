...
Supports optional qmd integration for semantic search.
"""
...
class QmdIntegration:
    """Integration with qmd for semantic search."""
    ...
    @staticmethod
    def collection_exists(collection: str = 'context') -> bool:
        ...
        try:
            ...
            return collection in result.stdout
        except Exception:
            return False
    ...
    def search(
        ...
        """
        Perform semantic search using qmd.
        Returns None if qmd is not available or search fails.
        """
        if not QmdIntegration.is_available():
            return None

        if not QmdIntegration.collection_exists(collection):
            return None

        try:
            result = subprocess.run(
                ['qmd', 'query', query, '--collection', collection, '--top-k', str(top_k), '--json'],
                capture_output=True,
                text=True,
                timeout=30
            )
            ...
            if result.returncode == 0:
                ...
                return [
                    {
                        'file': item.get('path', ''),
                        'category': Path(item.get('path', '')).parent.name,
...
def main():
    ...
    parser.add_argument(
        '--prefer-semantic',
        action='store_true',
        help='Prefer semantic search (qmd) if available'
    )
    ...
    try:
        ...
        # Try semantic search first if preferred and available
        if args.prefer_semantic and args.keywords:
            query = ' '.join(args.keywords)
            results = QmdIntegration.search(
                query=query,
